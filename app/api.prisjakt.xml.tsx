...
export async function loader({ request }: LoaderFunctionArgs) {
  // Enkel signatur: FEED_SECRET i .env
  const url = new URL(request.url);
  const base = url.searchParams.get("base");                 // ex: https://nordicaim.se
  const tag  = (url.searchParams.get("tag") || "prisjakt").toLowerCase();
  const sig  = url.searchParams.get("sig");

  if (!base || !sig) return new Response("Missing base or sig", { status: 400 });
  if (!process.env.FEED_SECRET || sig !== process.env.FEED_SECRET) {
    return new Response("Unauthorized", { status: 401 });
  }

  // Admin-session från Shopify Remix-helpern
  ...
  const itemsXml = filtered.map((p: any) => {
    ...
    const link = `${base.replace(/\/$/, "")}/products/${p.handle}`;
